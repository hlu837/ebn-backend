...
class _CustomerHomeScreenState extends State<CustomerHomeScreen> {
  ...
  void initState() {
    ...
    _unreadPollTimer = Timer.periodic(
        const Duration(seconds: 5), (_) => _refreshUnreadCount());
...
class _IconCircleButton extends StatelessWidget {
  const _IconCircleButton(
      {required this.icon, this.onTap, this.badgeCount = 0});
...
class _CategoryTabStrip extends StatelessWidget {
  const _CategoryTabStrip(
      {required this.tabs, required this.activeIndex, required this.onChanged});
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(AppRadii.md),
          border:
              Border.all(color: AppColors.primaryYellow.withValues(alpha: 0.5)),
        ),
        child: Row(
          children: [
            Container(
