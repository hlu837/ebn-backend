...
class AdminSettingsService {
  ...
  Future<List<AdminCategory>> fetchCategories({required String token}) async {
    ...
    final rows = (json['categories'] as List<dynamic>).cast<Map<String, dynamic>>();
  ...
  Future<List<AdminCategory>> reorderCategories(List<String> orderedIds, {required String token}) async {
    final json = await _put('/api/admin-settings/categories/reorder', {'orderedIds': orderedIds}, token: token);
    final rows = (json['categories'] as List<dynamic>).cast<Map<String, dynamic>>();
    return rows.map(AdminCategory.fromJson).toList();
  }
  ...
  Future<AdminCity> createCity({required String name, bool isLive = true, required String token}) async {
    final json = await _post('/api/admin-settings/cities', {'name': name, 'isLive': isLive}, token: token);
    return AdminCity.fromJson(json);
  }

  Future<AdminCity> updateCity(String id, {String? name, bool? isLive, required String token}) async {
    final json = await _patch(
  ...
  Future<AdminFaqEntry> createFaq({required String question, required String answer, required String token}) async {
    final json = await _post('/api/admin-settings/faq', {'question': question, 'answer': answer}, token: token);
  ...
  Future<List<AdminContentPage>> fetchContentPages({required String token}) async {
  ...
  Future<AdminContentPage> updateContentPage(
    ...
    final json = await _put('/api/admin-settings/content-pages/$pageKey', {'title': title, 'body': body}, token: token);
  ...
  Future<AdminAccountSummary> inviteAdmin({
    ...
    final json = await _post(
      ...
      {'fullName': fullName, 'email': email, 'password': password, if (phone != null) 'phone': phone},
      token: token,
    );
  ...
  Future<AdminGeneralSettings> fetchGeneralSettings({required String token}) async {
  ...
  Future<Map<String, dynamic>> _get(String path, {String? token}) async {
    ...
    try {
      ...
      throw const AdminSettingsServiceException("Couldn't reach the server. Check your connection and try again.");
    }
    return _decode(res);
  }
