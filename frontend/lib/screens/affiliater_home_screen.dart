...
class _AffiliaterHomeScreenState extends State<AffiliaterHomeScreen> {
  ...
  void _copyCode() {
    ...
    AppToast.showSuccess(context, 'Referral link copied: $shareUrl');
  }
  ...
  void _requestPayout() async {
    ...
    AppToast.showSuccess(context, 'Payout requested. Admin will review it shortly.');
  }
  ...
  void _generateLink(Asset asset) async {
    ...
    if (token != null && token.isNotEmpty) {
      try {
        final linkData = await _affiliateService.generateLink(token, assetId: asset.id);
        url = linkData['url'] ?? url;
      } catch (_) {}
    }
    await Clipboard.setData(ClipboardData(text: url));
    if (!mounted) return;
    AppToast.showSuccess(context, 'Link copied for "${asset.title}": $url');
  }
      ...
      builder: (_) => _MoreBottomSheet(
        onReports: () {
          ...
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateReportsScreen(token: widget.user.token ?? '')));
        },
        onMembership: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(builder: (_) => AffiliateMembershipScreen(user: widget.user)));
        },
        onSettings: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateAccountSettingsScreen(user: widget.user)));
        },
        onSupport: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(builder: (_) => AffiliateSupportScreen(user: widget.user)));
        },
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  _AffiliateCodeCard(code: _affiliateCode, onCopy: _copyCode, onRequestPayout: _requestPayout),
                  const SizedBox(height: AppSpacing.lg),
                  _QuickActionsRow(
                    onShareLink: _copyCode,
                    onAssets: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AffiliatePropertiesScreen())),
                    onCampaigns: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AffiliateCampaignsScreen())),
                    onReferrals: () => Navigator.of(context).push(MaterialPageRoute(
                        builder: (_) => AffiliateReferralsScreen(token: widget.user.token ?? ''))),
                    onTokens: () => Navigator.of(context).push(MaterialPageRoute(
                        builder: (_) => AffiliateTokensScreen(token: widget.user.token ?? ''))),
                  ),
                  const SizedBox(height: AppSpacing.xl),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('Top Properties', style: Theme.of(context).textTheme.titleMedium),
                      TextButton(
                        onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AffiliatePropertiesScreen())),
                        child: const Text('View All', style: TextStyle(color: _kAccentRed, fontWeight: FontWeight.w700)),
                      ),
                    ],
                  ),
                  ...
                  const _PromoBanner(),
                  const SizedBox(height: AppSpacing.xl),
                  Text('Referral Tracker', style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(height: AppSpacing.md),
                  _TrackerTabStrip(
                  ...
                  if (_filteredReferrals.isEmpty)
                    const Padding(
                      ...
                      child: Center(
                        child: Text('Nothing here yet.', style: TextStyle(color: AppColors.slate, fontWeight: FontWeight.w600)),
                      ),
                    ),
                ],
              ),
      ...
      bottomNavigationBar: _BottomNavBar(
        ...
        onChanged: (i) {
          if (i == 1) {
            Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AffiliatePropertiesScreen()));
            return;
          }
          if (i == 2) {
            Navigator.of(context).push(MaterialPageRoute(
                builder: (_) => AffiliateEarningsScreen(token: widget.user.token ?? '')));
            return;
          }
          if (i == 3) {
            Navigator.of(context).push(MaterialPageRoute(
              ...
              if (unreadNotifications > 0)
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: const BoxDecoration(color: _kAccentRed, shape: BoxShape.circle),
...
class _AffiliateCodeCard extends StatelessWidget {
  const _AffiliateCodeCard({required this.code, required this.onCopy, required this.onRequestPayout});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x33E84C3D), blurRadius: 16, offset: Offset(0, 8))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('My Affiliate Code', style: TextStyle(color: Colors.white70, fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(height: AppSpacing.sm),
          Container(
            width: double.infinity,
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(code, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.ink, letterSpacing: 0.5)),
                ),
                InkWell(
                  onTap: onCopy,
                  ...
                  child: const Padding(
                    ...
                    child: Icon(Icons.copy_rounded, size: 20, color: _kAccentRed),
                  ),
                ),
              ],
            ),
...
class _QuickActionsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: items
          .map((item) => _QuickActionButton(icon: item.$1, label: item.$2, onTap: item.$3))
          .toList(),
    );
  }
}
...
class _AffiliatePropertyCard extends StatelessWidget {
  const _AffiliatePropertyCard({required this.asset, required this.commissionPercent, required this.onGenerateLink});
          ...
          children: [
              ...
              children: [
                AspectRatio(
                  ...
                  child: asset.imageUrl != null
                      ? Image.network(asset.imageUrl!, fit: BoxFit.cover, width: double.infinity)
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: AppColors.success, borderRadius: BorderRadius.circular(AppRadii.pill)),
                    child: Text(
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(asset.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: AppColors.ink)),
                  const SizedBox(height: 2),
                  Text(asset.formattedPrice, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w800, color: _kAccentRed)),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: _kAccentRed, borderRadius: BorderRadius.circular(AppRadii.pill)),
                  child: const Text('Limited Offer', style: TextStyle(color: Colors.white, fontSize: 9.5, fontWeight: FontWeight.w800)),
                ),
                const SizedBox(height: 6),
                const Text('Summer Real Estate Drive', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.ink)),
                const SizedBox(height: 2),
                const Text('Earn Double Commission (4%)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.slate)),
              ],
            ),
          ),
          Container(
        ...
        return Padding(
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Text(
                _labels[i],
                style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: active ? Colors.white : AppColors.slate),
              ),
            ),
          ),
        );
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(entry.customerName, style: const TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: AppColors.ink)),
                const SizedBox(height: 2),
                Text(entry.propertyTitle, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w500, color: AppColors.slate)),
              ],
            ),
          ),
          Column(
            ...
            children: [
              Text('${entry.commissionAmount.toStringAsFixed(0)} ETB', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: AppColors.ink)),
              const SizedBox(height: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(
                  color: entry.isPending ? const Color(0xFFFFF3D6) : const Color(0xFFE3F6EA),
                  borderRadius: BorderRadius.circular(AppRadii.pill),
                ),
                child: Text(
                  ...
                  style: TextStyle(
                    ...
                    color: entry.isPending ? const Color(0xFFB8860B) : AppColors.success,
                  ),
                ),
              ),
            ],
...
class _BottomNavBar extends StatelessWidget {
  ...
  static const _items = [
    ...
    (Icons.holiday_village_outlined, Icons.holiday_village_rounded, 'Properties'),
          ...
          children: List.generate(_items.length, (i) {
            ...
            return InkWell(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(active ? filled : outline, size: 22, color: active ? _kAccentRed : AppColors.slate),
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text('More', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.ink)),
          ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(subtitle, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.slate)),
                ],
              ),
            ),
