...
class _CategoryListingScreenState extends State<CategoryListingScreen> {
  ...
  Broker? _brokerFor(Asset asset) {
    ...
    for (final c in _allCategories) {
      final candidates = _allBrokers.where((b) => b.specialties.contains(c)).toList();
                  ...
                  if (_loading)
                    ...
                    Container(
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          const Icon(Icons.cloud_off_rounded, size: 34, color: LandingColors.muted),
                          const SizedBox(height: 12),
                          Text(_error!, textAlign: TextAlign.center, style: const TextStyle(color: LandingColors.muted)),
                                  ...
                                  onBrokerAvatarTap: () {
                                    ...
                                    if (broker == null) {
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(content: Text("No agent info for this listing yet.")),
                                      );
                                      return;
                                    }
...
class _BrokerFinderSheet extends StatelessWidget {
  ...
  const _BrokerFinderSheet({required this.category, required this.categoryLabel, required this.brokers});
