...
class AffiliatePropertiesScreen extends StatefulWidget {
  const AffiliatePropertiesScreen({super.key});

  @override
  State<AffiliatePropertiesScreen> createState() => _AffiliatePropertiesScreenState();
}
...
class _AffiliatePropertiesScreenState extends State<AffiliatePropertiesScreen> {
  ...
  List<Asset> get _filtered {
    ...
    if (_query.trim().isNotEmpty) {
      ...
      list = list.where((a) => a.title.toLowerCase().contains(q) || (a.city ?? '').toLowerCase().contains(q));
    }
    return list.toList();
  }

  void _generateLink(Asset asset) {
    AppToast.showSuccess(context, 'Referral link generated for "${asset.title}".');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Properties', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
      ),
      body: Column(
        children: [
          Padding(
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                _CategoryChip(label: 'All', active: _category == null, onTap: () => setState(() => _category = null)),
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Text(
          label,
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: active ? Colors.white : AppColors.slate),
        ),
      ),
    );
  }
...
class _PropertyGridCard extends StatelessWidget {
  const _PropertyGridCard({required this.asset, required this.commissionPercent, required this.onGenerateLink});
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(asset.city ?? '—', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: AppColors.slate)),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _kAccentRed.withOpacity(0.1),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.holiday_village_outlined, size: 40, color: AppColors.slate),
            const SizedBox(height: AppSpacing.md),
            const Text('No properties match your search.', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w700, color: AppColors.ink)),
          ],
        ),
      ),
    );
