...
class SellPropertyFormScreen extends StatefulWidget {
  const SellPropertyFormScreen({super.key, required this.user, this.isAgentListing = false});

  final AppUser user;

  /// True when an Agent is listing a property they own themselves (see
  /// `agent_home_screen.dart` / `agent_dashboard_screen.dart`'s "+" flow)
  /// instead of a Visitor submitting one for someone else to inspect.
  /// Adds a "Photos & Report" step (photos/video + written notes) before
  /// Review — the same media a claiming Agent would otherwise only add
  /// later via [AgentPropertyReportScreen] — and submits through
  /// [SellRequestController.submitAsAgent] so Admin can publish it
  /// directly under this Agent's name instead of opening it to the claim
  /// pool. Still pays the same 100 ETB fee.
...
class _SellPropertyFormScreenState extends State<SellPropertyFormScreen> {
  ...
  bool _validateCurrentStep() {
    ...
    if (widget.isAgentListing && _step == _lastStep - 1) {
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
          widget.isAgentListing
              ? 'Thanks! Your payment went through and your listing was sent to Admin for review. '
  ...
  void _addAgentMedia(bool isVideo) {
    setState(() => _agentMedia.add(ReportMediaItem(id: 'am${_agentMediaMockId++}', isVideo: isVideo)));
  }
  ...
  /// Agent self-listing only — the same photos/video + written report an
