...
class AffiliateReferralsScreen extends StatefulWidget {
  ...
  @override
  State<AffiliateReferralsScreen> createState() => _AffiliateReferralsScreenState();
}
...
class _AffiliateReferralsScreenState extends State<AffiliateReferralsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _error = 'Failed to load referrals.'; _loading = false; });
    }
  }
  ...
  List<ReferralItem> get _filtered {
    ...
    if (_query.trim().isNotEmpty) {
      ...
      list = list.where((r) => r.customerName.toLowerCase().contains(q) || r.assetTitle.toLowerCase().contains(q));
    }
    final result = list.toList()..sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return result;
  }

  double get _totalForFilter => _filtered.fold(0, (sum, r) => sum + r.commissionAmount);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('My Referrals', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
        actions: [
          IconButton(
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by customer or property',
                ...
                Row(
                  children: [
                    _FilterChip(label: 'All', active: _tabIndex == 0, onTap: () => setState(() => _tabIndex = 0)),
                    const SizedBox(width: AppSpacing.sm),
                    _FilterChip(label: 'Pending', active: _tabIndex == 1, onTap: () => setState(() => _tabIndex = 1)),
                    const SizedBox(width: AppSpacing.sm),
                    _FilterChip(label: 'Completed', active: _tabIndex == 2, onTap: () => setState(() => _tabIndex = 2)),
                  ],
                ),
                const SizedBox(height: AppSpacing.md),
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        ...
                        style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: AppColors.slate),
                      ),
                      Text(
          ...
          Expanded(
            child: filtered.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, i) => _ReferralRow(entry: filtered[i]),
                  ),
          ),
        ],
      ),
...
class _ReferralRow extends StatelessWidget {
  ...
  String get _formattedDate => '${_months[entry.createdAt.month - 1]} ${entry.createdAt.day}, ${entry.createdAt.year}';
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(entry.customerName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.ink)),
                const SizedBox(height: 2),
                Text(entry.assetTitle, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.slate)),
                const SizedBox(height: 2),
                Text(_formattedDate, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: AppColors.slate)),
              ],
            ),
          ),
          Column(
            ...
            children: [
              Text('${entry.commissionAmount.toStringAsFixed(0)} ${entry.commissionCurrency}',
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.groups_2_outlined, size: 40, color: AppColors.slate),
            const SizedBox(height: AppSpacing.md),
            const Text('No referrals here yet.', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w700, color: AppColors.ink)),
