...
// PATCH /api/agents/:agentId/profile — Body: { avatarUrl?, bio?, city?, addressLine?, specialties? }
router.patch(
  '/:agentId/profile',
  ...requireOwner,
  asyncHandler(async (req, res) => {
    const { avatarUrl, bio, city, addressLine, specialties } = req.body || {};
    ...
    if (addressLine !== undefined && addressLine !== null &&
        (typeof addressLine !== 'string' || addressLine.length > 200)) {
      return res.status(400).json({ error: 'addressLine must be a string no longer than 200 characters.' });
    }
    ...
    const row = await profileModel.update(req.params.agentId, { avatarUrl, bio, city, addressLine, specialties });
