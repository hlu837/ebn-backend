...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // toPublicAsync (not toPublic) — reads live admin-configured prices
    // from the membership_pricing table instead of the hardcoded fallback.
    res.json({ ...(await membershipModel.toPublicAsync(membershipRow)), billingHistory: billing });
  })
);

// POST /api/affiliates/me/membership/upgrade — Body: { tier }
// Only reachable for tiers with no charge (shouldn't normally happen,
// since every tier above bronze costs something) — paid tiers must go
// through Chapa checkout first; see activateAffiliateMembershipUpgrade
// in routes/payments.js, which is what actually applies a paid upgrade
// once Chapa confirms the payment. This keeps a client from granting
// itself a paid tier by just calling this endpoint directly.
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const pricing = await membershipModel.getCachedTierPricing();
    const fee = pricing[tier] ?? membershipModel.TIER_MONTHLY_FEE_ETB[tier];
    if (fee > 0) {
      return res.status(402).json({
        error: 'This plan requires payment. Start checkout with POST /api/payments/chapa/initialize ' +
          `(purpose: "affiliate_membership_${tier}") — the upgrade is applied automatically once Chapa confirms the payment.`,
      });
    }
    const row = await membershipModel.setTier(req.user.id, tier);
    res.json(await membershipModel.toPublicAsync(row));
  })
);
...
// ── Expert profile (Expert merged into the Affiliater dashboard) ──────────
// An Affiliater who wants to also take maintenance jobs links their
// account to a service_providers row (086_service_provider_user_link.sql)
// instead of signing up separately — "Expert" isn't its own role, it's a
// capability an Affiliater turns on. Creating a profile here always
// starts inactive; an admin flips is_active in Admin Settings → Service
// Providers before the profile shows up in the tenant-facing directory,
// same gate the admin-entered directory already uses.

// GET /api/affiliates/me/expert-profile
router.get(
  '/me/expert-profile',
  ...requireAffiliater,
  asyncHandler(async (req, res) => {
    const row = await serviceProvidersModel.findByUserId(req.user.id);
    res.json({ profile: row ? serviceProvidersModel.toPublic(row) : null });
  })
);

// POST /api/affiliates/me/expert-profile — becomes an Expert.
router.post(
  '/me/expert-profile',
  ...requireAffiliater,
  asyncHandler(async (req, res) => {
    const { name, category, phone, city, latitude, longitude, rateCents } = req.body || {};
    const row = await serviceProvidersModel.createSelf(req.user.id, {
      name,
      category,
      phone,
      city,
    ...
    res.status(201).json({ profile: serviceProvidersModel.toPublic(row) });
  })
);

// PUT /api/affiliates/me/expert-profile — edit own profile (not isActive).
router.put(
  '/me/expert-profile',
  ...requireAffiliater,
  asyncHandler(async (req, res) => {
    const { name, category, phone, city, latitude, longitude, rateCents } = req.body || {};
    const row = await serviceProvidersModel.updateSelf(req.user.id, {
      name,
      category,
      phone,
      city,
    ...
    res.json({ profile: serviceProvidersModel.toPublic(row) });
  })
);

// GET /api/affiliates/me/expert-jobs — jobs assigned to this Expert's linked provider row.
router.get(
  '/me/expert-jobs',
  ...requireAffiliater,
  asyncHandler(async (req, res) => {
    const provider = await serviceProvidersModel.findByUserId(req.user.id);
    if (!provider) return res.json([]);
    const rows = await maintenanceAssignmentsModel.listForProvider(provider.id);
    res.json(rows.map(maintenanceAssignmentsModel.toPublic));
  })
);

// GET /api/affiliates/me/expert-wallet — payout ledger + balance for this Expert.
router.get(
  '/me/expert-wallet',
  ...requireAffiliater,
  asyncHandler(async (req, res) => {
    const provider = await serviceProvidersModel.findByUserId(req.user.id);
    if (!provider) return res.json({ balanceCents: 0, transactions: [] });
    const [summary, transactions] = await Promise.all([
      specialistWalletModel.getSummary(provider.id),
      specialistWalletModel.listByProvider(provider.id),
    ]);
    res.json({ balanceCents: summary.balanceCents, transactions: transactions.map(specialistWalletModel.toPublic) });
  })
);
