...
/**
 * Closes the loop for a rental agreement's payment — purpose tag
 * `rental_agreement_<id>` (see backend/src/models/rentalAgreements.js).
 * Mirrors activatePaidSignup's shape: a no-op for any other purpose,
 * called unconditionally alongside it from both verify and webhook.
 */
async function activateRentalAgreementPayment(payment) {
  if (payment.status !== 'success') return;
  if (!payment.purpose || !payment.purpose.startsWith('rental_agreement_')) return;
  const agreementId = payment.purpose.slice('rental_agreement_'.length);
  try {
    const paid = await require('../models/rentalAgreements').markPaid(agreementId, payment.tx_ref);
    if (paid) {
      require('../scheduler').cancel(agreementId);
      const rentalAgreementsRouter = require('./rentalAgreements');
      rentalAgreementsRouter.cancelPaymentReminder(agreementId);
      rentalAgreementsRouter.armLeaseReminders(paid);
    }
  } catch (err) {
    console.error('[payments] failed to mark rental agreement paid', err);
  }
}

/**
 * Closes the loop for a maintenance job's escrow payment — purpose tag
 * `maintenance_escrow_<assignment id>` (see
 * backend/src/models/maintenanceAssignments.js). Same shape as
 * activateRentalAgreementPayment: a no-op for any other purpose, called
 * unconditionally alongside it from both verify and webhook.
 */
async function activateMaintenanceEscrowPayment(payment) {
  if (payment.status !== 'success') return;
  if (!payment.purpose || !payment.purpose.startsWith('maintenance_escrow_')) return;
  const assignmentId = payment.purpose.slice('maintenance_escrow_'.length);
  try {
    await require('../models/maintenanceAssignments').markHeld(assignmentId, payment.tx_ref);
  } catch (err) {
    console.error('[payments] failed to mark maintenance escrow held', err);
  }
}

/**
 * Closes the loop for an affiliate's paid membership-tier upgrade —
 * purpose tag `affiliate_membership_<tier>` (see
 * affiliate_membership_screen.dart / affiliate_service.dart on the
 * client, and the guarded POST /me/membership/upgrade route above in
 * routes/affiliates.js, which now refuses to apply a paid tier directly
 * for exactly this reason). Same no-op-for-any-other-purpose shape as
 * the other activate* helpers, called unconditionally from both verify
 * and webhook.
 */
async function activateAffiliateMembershipUpgrade(payment) {
  if (payment.status !== 'success') return;
  if (!payment.purpose || !payment.purpose.startsWith('affiliate_membership_')) return;
  const tier = payment.purpose.slice('affiliate_membership_'.length);
  if (!affiliateMembershipModel.TIERS.includes(tier)) return;
  const userId = payment.owner_user_id;
  if (!userId) return;
  // Belt-and-braces: even though /chapa/initialize now overrides the
  // amount for this purpose server-side (so this really shouldn't ever
  // fire), double-check the amount actually paid matches the real tier
  // price before granting it — in case this payment row was created by
  // some other path that didn't go through that guard.
  try {
    const pricing = await affiliateMembershipModel.getCachedTierPricing();
    const realFee = pricing[tier] ?? affiliateMembershipModel.TIER_MONTHLY_FEE_ETB[tier];
    if (Number(payment.amount) < Number(realFee)) {
      console.error('[payments] affiliate membership payment amount mismatch — refusing to grant tier', {
        userId,
        tier,
        paid: payment.amount,
        expected: realFee,
    ...
    console.error('[payments] failed to verify affiliate membership payment amount', err);
    return;
  }
  try {
    await affiliateMembershipModel.setTier(userId, tier);
  } catch (err) {
    console.error('[payments] failed to apply affiliate membership upgrade', err);
  }
}
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Rental agreements are bank-transfer-only for now — see
    // activateRentalAgreementPayment below, which still exists to close
    // out any older Chapa-paid rows, but no new one should be started.
    if (String(body.purpose || '').startsWith('rental_agreement_')) {
      return res.status(422).json({ error: 'Rental payments are made by bank transfer — see the agreement\'s bank accounts.' });
    }

    // Affiliate membership upgrades: the client-supplied `amount` is
    // NEVER trusted for this purpose — a tampered client could otherwise
    // request `affiliate_membership_diamond` with `amount: 1` and still
    // get the Diamond tier once Chapa confirms that tiny payment (see
    // activateAffiliateMembershipUpgrade, which only checks that the
    // payment succeeded, not what it was for). The real, admin-configured
    // price is looked up here instead and silently overrides whatever
    // the client sent.
    let amountOverride;
    if (String(body.purpose || '').startsWith('affiliate_membership_')) {
      const tier = String(body.purpose).slice('affiliate_membership_'.length);
      if (!affiliateMembershipModel.TIERS.includes(tier)) {
        return res.status(400).json({ error: `Unknown membership tier "${tier}".` });
      }
      const pricing = await affiliateMembershipModel.getCachedTierPricing();
      const realFee = pricing[tier] ?? affiliateMembershipModel.TIER_MONTHLY_FEE_ETB[tier];
      if (!(realFee > 0)) {
        return res.status(400).json({ error: `${tier} has no charge — upgrade it directly instead of paying for it.` });
      }
      amountOverride = realFee;
    }
    ...
    const amount = amountOverride ?? Number(body.amount);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    await activateRentalAgreementPayment(updated);
    await activateMaintenanceEscrowPayment(updated);
    await activateAffiliateMembershipUpgrade(updated);
