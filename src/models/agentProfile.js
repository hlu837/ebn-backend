...
function update(userId, { avatarUrl, bio, city, addressLine, specialties }) {
  return query(
    `INSERT INTO agent_profiles (user_id, avatar_url, bio, city, address_line, specialties)
    VALUES ($1, $2, COALESCE($3, ''), COALESCE($4, ''), $5, COALESCE($6::text[], '{}'::text[]))
     ON CONFLICT (user_id) DO UPDATE
       SET avatar_url = COALESCE($2, agent_profiles.avatar_url),
           ...
           address_line = COALESCE($5, agent_profiles.address_line),
           specialties = COALESCE($6::text[], agent_profiles.specialties)
     RETURNING *`,
    [userId, avatarUrl ?? null, bio ?? null, city ?? null, addressLine ?? null, specialties ?? null]
  ).then((r) => r.rows[0]);
}
...
async function listDirectory({ specialty, city, search, excludeUserId, userId } = {}) {
  ...
  const { rows } = await query(
    `SELECT
      ...
      p.avatar_url, p.bio, p.city, p.address_line, p.specialties, p.boosted,
