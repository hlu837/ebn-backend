...
async function toPublicAsync(row) {
  ...
  return {
    ...
    perks: TIER_PERKS[row.tier],
    // Full per-tier price map (admin-configurable via
    // membership_pricing) so the client can show correct prices for
    // *every* tier in the upgrade picker, not just the current one.
    tierPricing: { ...TIER_MONTHLY_FEE_ETB, ...pricing },
  };
}
...
/** Called after an admin price change so the next request re-fetches from
 * the DB instead of serving the stale in-memory cache for up to 60s. */
function invalidatePricingCache() {
  lastPricingFetch = 0;
}

module.exports = { TIERS, TIER_MONTHLY_FEE_ETB, TIER_PERKS, toPublic, toPublicAsync, getOrCreate, listBilling, setTier, getTierPricing, getCachedTierPricing, invalidatePricingCache };
