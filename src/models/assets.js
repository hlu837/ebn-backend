...
const { makeThumbnailDataUrl } = require('../utils/thumbnail');
...
/** Same shape as `toPublic`, but `image_url` is swapped for the small
 *  cached `thumbnail_url` (falling back to the real `image_url` for a row
 *  whose thumbnail hasn't been backfilled yet — see
 *  scripts/backfillAssetThumbnails.js) and `image_urls` is dropped
 *  entirely. Used ONLY by the browse-feed list endpoint (GET /api/assets)
 *  — every card/rail across the app renders `imageUrl` alone, never the
 *  full gallery, so this is enough to keep listing feeds looking the
 *  same while cutting the payload from megabytes to a few KB per row.
 *  Detail views and the broker-scoped endpoints keep using `toPublic`. */
function toPublicList(row) {
  const full = toPublic(row);
  if (!full) return null;
  return {
    ...full,
...
async function create({
  ...
  const thumbnail = await makeThumbnailDataUrl(cover);
  ...
  return query(
    `INSERT INTO assets (
       ...
       image_urls, thumbnail_url, posted_label, broker_id, rating, review_count, roi_percent
     )
     VALUES ($1, $2, $3, $4, $5, $6::asset_status, $7, $8, $9, $10, $11::jsonb, $12, $13::jsonb, $14, $15, $16, $17, $18, $19)
     RETURNING *`,
    [
      title,
      description || null,
...
async function update(id, patch = {}) {
  ...
  // Cover photo changed (or was cleared) — regenerate the cached list
  // thumbnail so it never drifts out of sync with `image_url`. Skipped
  // entirely when `imageUrl` isn't part of this patch, so ordinary edits
  // (price, title, status, ...) don't pay for a re-encode they don't need.
  if (Object.prototype.hasOwnProperty.call(patch, 'imageUrl')) {
    const thumbnail = await makeThumbnailDataUrl(patch.imageUrl);
    sets.push(`thumbnail_url = $${params.push(thumbnail)}`);
  }
