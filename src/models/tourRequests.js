...
// Includes rows currently broadcasting to this agent (agent_id is NULL
// while broadcasting, so those would otherwise vanish from this agent's
// view entirely — they'd only show in the transient home-screen banner,
// which the frontend's own status-chip handling for 'broadcasting' was
// already built to expect here).
function listHistoryForAgent(agentId) {
  return query(
    `SELECT * FROM tour_requests
     WHERE agent_id = $1
        OR (status = 'broadcasting' AND $1 = ANY(broadcast_agent_ids))
