...
const visitorSettingsModel = require('./visitorSettings');
...
/**
 * True unless the recipient is a visitor who's explicitly turned off
 * "Sell & order request updates" (visitor account settings screen).
 * Non-visitor recipients (agent/admin/etc., shouldn't normally land here
 * but just in case) always get notified — that preference only exists
 * on visitor_settings.
 */
async function shouldNotifyRequester(recipientType, recipientId) {
  if (recipientType !== 'user') return true;
  try {
    const settings = visitorSettingsModel.toPublic(await visitorSettingsModel.getOrCreate(recipientId));
    return !settings || settings.notifyRequestUpdates !== false;
  } catch (err) {
    // Fail open — same reasoning as the agent-side notifyNewDispatches
    // check just above: a broken preference lookup shouldn't silently
    // swallow a real status update the requester is waiting on.
    // eslint-disable-next-line no-console
    console.error(`[orderRequests] failed to load visitor settings for ${recipientId}`, err);
    return true;
  }
}
...
async function notifyRequesterOfClaim(orderRequestRow) {
  ...
  try {
    ...
    if (!(await shouldNotifyRequester(recipientType, String(orderRequestRow.requester_user_id)))) return;
