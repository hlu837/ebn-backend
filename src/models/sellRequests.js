 ...
 * "Sell & order request updates" (visitor account settings screen). See
 * the identical helper in orderRequests.js — kept local to each file
 * rather than shared since neither model otherwise depends on the other.
 */
async function shouldNotifyRequester(recipientType, recipientId) {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error(`[sellRequests] failed to load visitor settings for ${recipientId}`, err);
    return true;
  }
}
...
async function notifyOwnerOfClaim(row) {
  ...
  try {
    ...
    if (!(await shouldNotifyRequester(recipientType, String(row.owner_user_id)))) return;
