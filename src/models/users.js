...
/**
 * Creates a user whose account needs admin sign-off but no payment —
 * used for Property Owner signup. Same "reserve the email immediately"
 * shape as createPending(), but starts at 'pending_approval' instead of
 * 'pending_payment' so it skips the payments flow entirely. An admin
 * later calls activatePendingRole() (approve) or rejectPendingRole()
 * (reject) to resolve it.
 */
function createPendingApproval({ fullName, email, passwordHash, pendingRole, phone }) {
  return query(
    `INSERT INTO users (
       full_name, email, password_hash, role,
       phone, account_status, pending_role
     )
     VALUES ($1, $2, $3, 'user', $4, 'pending_approval', $5)
     RETURNING *`,
    [fullName, email, passwordHash, phone || null, pendingRole]
  ).then((r) => r.rows[0]);
}
 ...
 */
function listAll({ role, accountStatus, search, limit = 20, offset = 0 } = {}) {
  const conditions = [];
  const params = [];
...
function count({ role, accountStatus, search } = {}) {
  const conditions = [];
  const params = [];
...
/**
 * Stores the hash of a freshly-issued "forgot password" reset code plus
 * its expiry — see migrations/071_password_reset.sql for why only the
 * hash is persisted.
 */
function setResetToken(id, tokenHash, expiresAt) {
  return query(
    `UPDATE users
...
/** Clears a reset code once it's been used (or superseded by a new request). */
...
/**
 * Stores the hash of a freshly-issued "verify your email" code plus its
 * expiry — same shape as setResetToken(), see
 * migrations/072_email_verification.sql for why only the hash is persisted.
 */
function setVerificationToken(id, tokenHash, expiresAt) {
  return query(
    `UPDATE users
     SET email_verify_token_hash = $2, email_verify_expires_at = $3
     WHERE id = $1
     RETURNING *`,
    [id, tokenHash, expiresAt]
  ).then((r) => r.rows[0] || null);
...
/** Marks the account verified and clears the now-used code. */
...
/**
 * Admin rejects a pending-approval account (e.g. a Property Owner
 * request). Demotes it back to a plain active Visitor rather than
 * deleting it, since the email/password are already real.
 */
function rejectPendingRole(id, role) {
  return query(
    `UPDATE users
     SET role = 'user', account_status = 'active', pending_role = NULL
     WHERE id = $1 AND account_status = 'pending_approval' AND pending_role = $2
     RETURNING *`,
    [id, role]
  ).then((r) => r.rows[0] || null);
}
