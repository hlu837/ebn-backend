...
function threadToPublic(row) {
  ...
  return {
    ...
    // Present only when this thread is scoped to a listing — general
    // (asset-less) threads leave asset_id null so the client can show a
    // "General inquiry" style header instead of a broken listing card.
    asset:
      row.asset_id != null && row.asset_title !== undefined
...
function messageToPublic(row) {
  ...
  return {
    ...
    body: row.body,
    createdAt: row.created_at,
    // Optional typed link (e.g. kind 'rental_agreement' + the agreement's
    // id) — lets the client show an action button under the bubble.
    // Both are null for ordinary messages.
    kind: row.kind ?? null,
 ...
 *
 * `assetId` is optional — a null value opens/reuses the one "general"
 * thread between this customer and agent (see migration 073), used when a
 * visitor taps "Chat" on a broker's profile rather than on a listing.
 */
function getOrCreateThread({ customerId, agentId, assetId }) {
  ...
  return query(
    `INSERT INTO chat_threads (customer_id, agent_id, asset_id)
     VALUES ($1, $2, NULL)
     ON CONFLICT (customer_id, agent_id) WHERE asset_id IS NULL
 ...
 *
 * `kind` / `relatedId` / `meta` are optional and only used by system-generated
 * messages (see migration 093). Ordinary user messages never set them, and
 * for those the INSERT is exactly the original three-column statement.
 */
async function sendMessage({ threadId, senderId, body, kind = null, relatedId = null, meta = null }) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const inserted = kind
      ? await client.query(
          `INSERT INTO chat_messages (thread_id, sender_id, body, kind, related_id, meta)
           VALUES ($1, $2, $3, $4, $5, $6::jsonb) RETURNING *`,
          [threadId, senderId, body, kind, relatedId, meta ? JSON.stringify(meta) : null]
        )
      : await client.query(
