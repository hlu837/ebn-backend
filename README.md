...
3. Add these environment variables in Vercel:
   ...
   - `CRON_SECRET` — required for `GET /api/cron/sweep` below. Vercel sets this for you automatically the moment `vercel.json` has a `crons` entry (it's how Vercel authenticates its own cron calls to your app) — you don't need to set it by hand unless you also want to trigger the sweep from somewhere else (see below), in which case set it to the same value yourself and use it there too.
4. Run the database migrations (`npm run migrate`) against `DATABASE_URL` before or right after the first deploy — the app doesn't run them for you.

#### Limitations on Vercel — read this before relying on it in production

- **WebSocket / Socket.IO is not supported as a persistent live transport.** Every screen still works via its existing polling fallback (e.g. the owner review queue polls every 10s), but anything that depended on an instant push notification will feel slower — up to that poll interval — instead of instant.
- **In-memory timers (`scheduler.js`) do not run reliably in serverless production** — nothing keeps the Node process alive between requests for a `setTimeout` scheduled hours or days out to ever fire. `GET /api/cron/sweep` (added in this build, protected by `CRON_SECRET`) is the fix for this: it re-derives "what's due right now" from Postgres on every call instead of relying on a timer, and covers rental-agreement expiry/reminders, lease-lifecycle reminders, and investment payouts — all of which have windows measured in hours to days, so it's fine even at a coarse cadence. `vercel.json` wires it up on Vercel Cron at a conservative **once-daily** default (`0 3 * * *`) so it deploys on every plan, Hobby included — Hobby caps cron at once a day, and even that fires anywhere within the scheduled hour. That's too coarse for a 24-hour payment window to feel responsive; for real precision, either:
  - upgrade to Vercel Pro for per-minute cron cadence and tighten the schedule in `vercel.json`, or
  - leave the plan as-is and point an external scheduler (a GitHub Actions cron workflow, cron-job.org, EasyCron, etc.) at `https://<your-app>.vercel.app/api/cron/sweep` with an `Authorization: Bearer <CRON_SECRET>` header, on whatever cadence you actually want (e.g. every 15 minutes) — this works on any plan and doesn't require redeploying to change the schedule.
- **Tour-request agent dispatch does NOT work correctly on Vercel, and no cron configuration fixes this.** `DISPATCH_WINDOW_SECONDS` defaults to 30 seconds — a request that goes unanswered is supposed to fall through to the next nearby agent within that window. No cron product, Vercel's included even on its fastest paid tier, runs sub-minute reliably enough for a 30-second SLA. `GET /api/cron/sweep` still expires overdue dispatches as a coarse safety net so they don't get stuck forever, but agents will effectively lose their "first to accept" advantage and requests will sit far longer than intended before re-broadcasting. If this feature matters in production, either run this backend on a normal long-running host instead (Render, Railway, Fly.io, a VPS — no code changes needed, `scheduler.js`'s timers work as designed), or move just the dispatch logic to something that can hold a persistent process.
