...
if (!process.env.JWT_SECRET) {
  ...
  console.warn(
    '[server] JWT_SECRET is not set — auth tokens will fail to sign. Set it in .env (see .env.example).'
  );
}
...
async function rearmPendingExpiries() {
  const dispatched = await model.listDispatched();
...
httpServer.listen(PORT, async () => {
  ...
  // Automated investment payouts — see investmentPayoutScheduler.js.
  // Runs an initial sweep immediately, then hourly.
